...
export default function SerieTecnicoPage() {
  const t = useTranslations('SerieTecnico');
  const b = useTranslations('Breadcrumbs');

  const advantages = Array.from({ length: 5 }, (_, index) => t(`advantages.${index + 1}`));

  const characteristics = Array.from({ length: 1 }, (_, index) =>
  ...
  return (
    <>
      <div className="container mx-auto px-4 md:px-8">
        ...
        <ProductDetail
          title={t('title')}
          images={['/images/serie-tecnico.png']}
